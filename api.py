...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Adjust this to your frontend's domain if needed
...
async def predict(payload: PredictRequest) -> dict:
    ...
    # coverage is a list of [h3 cell, elevation in meters, loss in dB]
    # convert to a list of model RSSI as a function of lat, lon
    ...
    for row in coverage:
        center_h3, elevation, loss_db = row
        feature = {
            "type": "Feature",
            "geometry": {
                "type": "Point",
                "coordinates": h3.h3_to_geo(hex(center_h3)),  # latitude, longitude
            },
            "properties": {
                "elevation": elevation,
                "rssi": tx_power + payload.tx_gain + payload.rx_gain - loss_db,
            },
        }
